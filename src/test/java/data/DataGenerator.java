...
public class DataGenerator {
    ...
    public static String generateValidCity(String locale) {
        ...
        String city = citySubjectCenter[(int)Math.floor(Math.random() * citySubjectCenter.length)];
    ...
    public static String generateInvalidCity(String locale) {
        String citySubjectCenter[] = {"Люберцы", "Химки", "Париж", "Стокгольм", "Прага", "Осло"};
        String city = citySubjectCenter[(int)Math.floor(Math.random() * citySubjectCenter.length)];
        //  добавить логику для объявления переменной city и задания её значения, генерацию можно выполнить
        // с помощью Faker, либо используя массив валидных городов и класс Random
        return city;
